    ...
    <div class="a-order">
      ...
      >
        <div v-for="order in data.orderList" :key="order.id" class="order-item">
          ...
          <van-cell is-link>
            <template #title>
              <div style="display: flex;align-items: center;justify-content: space-between;" @click="gotoOrderDetail(order.id)">
                <div>
                ...
                <span :style="{color: order.statusStr.includes('关闭') ? '#e64340' : '#7F8487'}">{{ order.statusStr }}</span>
              </div>
            </template>
          </van-cell>
